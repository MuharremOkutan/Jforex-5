...
public class DataGen4  implements IStrategy, IFeedListener {
...
//                // EMA
//                double ema5 = indicators.ema(usedFeedDescriptor, IIndicators.AppliedPrice.CLOSE, usedOfferSide,
//                        5).calculate(0);
                getIndicatorValueThread ema5Obj = new getIndicatorValueThread(usedFeedDescriptor, indicators, usedOfferSide, "ema5");

                Thread ema5Thread = new Thread(ema5Obj);
                ema5Thread.start();
                ...
                double ema5 = ema5Obj.getDoubleVal();


                double ema20 = indicators.ema(usedFeedDescriptor, IIndicators.AppliedPrice.CLOSE, usedOfferSide,
                        20).calculate(0);

                // SMA
                double sma5 = indicators.sma(usedFeedDescriptor, IIndicators.AppliedPrice.CLOSE, usedOfferSide,
                        5).calculate(0);

                double sma20 = indicators.sma(usedFeedDescriptor, IIndicators.AppliedPrice.CLOSE, usedOfferSide,
                        20).calculate(0);

                // RSI
                double rsi14 = indicators.rsi(usedFeedDescriptor, IIndicators.AppliedPrice.CLOSE, usedOfferSide,
                        14).calculate(0);

                printer.printMe("here1");

                // Bollinger bands
                Double[] bbands20_2 = ArrayUtils.toObject(indicators.bbands(usedFeedDescriptor, IIndicators.AppliedPrice.CLOSE, usedOfferSide,
                        20, 2, 2, IIndicators.MaType.SMA).calculate(0));

                Double[] bbands20_1618 = ArrayUtils.toObject(indicators.bbands(usedFeedDescriptor, IIndicators.AppliedPrice.CLOSE, usedOfferSide,
                        20, 1.618, 1.618, IIndicators.MaType.SMA).calculate(0));

                Double[] bbands20_2618 = ArrayUtils.toObject(indicators.bbands(usedFeedDescriptor, IIndicators.AppliedPrice.CLOSE, usedOfferSide,
                        20, 2.618, 2.618, IIndicators.MaType.SMA).calculate(0));

                Double[] bbands20_4236 = ArrayUtils.toObject(indicators.bbands(usedFeedDescriptor, IIndicators.AppliedPrice.CLOSE, usedOfferSide,
                        20, 4.236, 4.236, IIndicators.MaType.SMA).calculate(0));

                printer.printMe("here2");

                // STOCHASTIC OSCILLATOR
                Double[] stoch5_3_3 = ArrayUtils.toObject(indicators.stoch(usedFeedDescriptor, usedOfferSide,
                        5, 3, IIndicators.MaType.SMA, 3, IIndicators.MaType.SMA).calculate(0));

                // MACD
                Double[] macd12_26_9 = ArrayUtils.toObject(indicators.macd(usedFeedDescriptor, IIndicators.AppliedPrice.CLOSE, usedOfferSide,
                        12, 26, 9).calculate(0));

                // ADX
                double adx14 = indicators.adx(usedFeedDescriptor, usedOfferSide,
                        14).calculate(0);

                printer.printMe("here3");

                // CCI
                double cci14 = indicators.cci(usedFeedDescriptor, usedOfferSide,
                        14).calculate(0);

                // ATR
                double atr14 = indicators.atr(usedFeedDescriptor, usedOfferSide,
                        14).calculate(0);

                // FRACTAL
                Double[] fractal5 = ArrayUtils.toObject(indicators.fractal(usedFeedDescriptor, usedOfferSide,
                        5).calculate(0));

                // SUPPORT AND RESISTANCE
                Double[] supRes = ArrayUtils.toObject(indicators.supportResistance(usedFeedDescriptor, usedOfferSide)
                        .calculate(0));
                ...
                // EMA
                rowData.ema5.put(feedType, ema5);
                rowData.ema20.put(feedType, ema20);
                // SMA
                rowData.sma5.put(feedType, sma5);
                rowData.sma20.put(feedType, sma20);
                // RSI
                rowData.rsi14.put(feedType, rsi14);
                // Bollinger bands
                rowData.bbands20_2.put(feedType, bbands20_2);
                rowData.bbands20_1618.put(feedType, bbands20_1618);
                rowData.bbands20_2618.put(feedType, bbands20_2618);
                rowData.bbands20_4236.put(feedType, bbands20_4236);
                // STOCHASTIC OSCILLATOR
                rowData.stoch5_3_3.put(feedType, stoch5_3_3);
                // MACD
                rowData.macd12_26_9.put(feedType, macd12_26_9);
                // ADX
                rowData.adx14.put(feedType, adx14);
                // CCI
                rowData.cci14.put(feedType, cci14);
                // ATR
                rowData.atr14.put(feedType, atr14);
                // FRACTAL
                rowData.fractal5.put(feedType, fractal5); // will always be N/A for shift 0 since it doesnt show up until 5 candles after
                // SUPPORT AND RESISTANCE
                rowData.supRes.put(feedType, supRes); // will always be N/A for shift 0 since it doesnt show up until 2 candles after
            }
...
class RowData {
    ...
    // EMA
    Map<FeedType, Double> ema5 = new HashMap<>();
    Map<FeedType, Double> ema20 = new HashMap<>();

    // SMA
    Map<FeedType, Double> sma5 = new HashMap<>();
    Map<FeedType, Double> sma20 = new HashMap<>();

    // RSI
    Map<FeedType, Double> rsi14 = new HashMap <>();

    // Bollinger bands
    Map<FeedType, Double[]> bbands20_2 = new HashMap <>();
    Map<FeedType, Double[]> bbands20_1618 = new HashMap <>();
    Map<FeedType, Double[]> bbands20_2618 = new HashMap <>();
    Map<FeedType, Double[]> bbands20_4236 = new HashMap <>();

    // STOCHASTIC OSCILLATOR
    Map<FeedType, Double[]> stoch5_3_3 = new HashMap <>();

    // MACD
    Map<FeedType, Double[]> macd12_26_9 = new HashMap <>();

    // ADX
    Map<FeedType, Double> adx14 = new HashMap <>();

    // CCI
    Map<FeedType, Double> cci14 = new HashMap <>();

    // ATR
    Map<FeedType, Double> atr14 = new HashMap <>();

    // FRACTAL
    Map<FeedType, Double[]> fractal5 = new HashMap <>();

    // SUPPORT AND RESISTANCE
    Map<FeedType, Double[]> supRes = new HashMap <>();
...
//            printer.printMe("----rowdata-----supRes  2: "+supRes.get(entry.getKey())[1]);
            ...
            String indicatorStr =
                    ...
                    // EMA
                    ema5.get(entry.getKey()) + "," +
                    ema20.get(entry.getKey()) + "," +

                    // SMA
                    sma5.get(entry.getKey()) + "," +
                    sma20.get(entry.getKey()) + "," +

                    // RSI - @return value for the specified bar
                    rsi14.get(entry.getKey()) + "," +

                    // Bollinger bands - @return values for the specified bars, outputs are returned in the following order: 'Upper Band', 'Middle Band', 'Lower Band'
                    bbands20_2.get(entry.getKey())[0] + "," +
                    bbands20_2.get(entry.getKey())[2] + "," +

                    bbands20_1618.get(entry.getKey())[0] + "," +
                    bbands20_1618.get(entry.getKey())[2] + "," +

                    bbands20_2618.get(entry.getKey())[0] + "," +
                    bbands20_2618.get(entry.getKey())[2] + "," +

                    bbands20_4236.get(entry.getKey())[0] + "," +
                    bbands20_4236.get(entry.getKey())[2] + "," +

                    // STOCHASTIC OSCILLATOR - @return values for the specified bars, outputs are returned in the following order: 'Slow %K', 'Slow %D'
                    stoch5_3_3.get(entry.getKey())[0] + "," +
                    stoch5_3_3.get(entry.getKey())[1] + "," +

                    // MACD - @return values for the specified bars, outputs are returned in the following order: 'MACD', 'MACD Signal', 'MACD Hist'
                    macd12_26_9.get(entry.getKey())[0] + "," +
                    macd12_26_9.get(entry.getKey())[1] + "," +
                    macd12_26_9.get(entry.getKey())[2] + "," +

                    // ADX - @return values for the specified bars
                    adx14.get(entry.getKey()) + "," +

                    // CCI - @return values for the specified bars
                    cci14.get(entry.getKey()) + "," +

                    // ATR
                    atr14.get(entry.getKey()) + "," +

                    // FRACTAL - @return values for the specified bars, outputs are returned in the following order: 'Maximums', 'Minimums'
                    fractal5.get(entry.getKey())[0] + "," +
                    fractal5.get(entry.getKey())[1] + "," +

                    // SUPPORT AND RESISTANCE - @return values for the specified bars, outputs are returned in the following order: 'Maximums', 'Minimums'
                    supRes.get(entry.getKey())[0] + "," +
                    supRes.get(entry.getKey())[1] + ",";
